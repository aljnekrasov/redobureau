...
// Liquid Redo 004: the legible cut. Same shell stack as 003, but the letter
// faces are filled solid-dark, the morph is calmer and the sway shallower,
// so the logotype stays readable while the contours keep breathing.
// Original notes:
...
const capFragmentShader = /* glsl */ `
  ...
  varying float vNoise;
  void main() {
    gl_FragColor = vec4(0.043, 0.075, 0.064, 1.0);
  }
`
...
new SVGLoader().load("redo-logo.svg", data => {
  ...
  const coreMesh = new THREE.Mesh(geometry, [caps, core])
...
function render() {
  ...
  group.rotation.y = Math.sin(morphTime * 0.14) * 0.12 + mouseX * 0.06
  group.rotation.x = Math.cos(morphTime * 0.11) * 0.06 - mouseY * 0.05
