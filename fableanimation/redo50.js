...
// Zelts: the mark as a solid gold ingot turning forever over a black mirror floor under one warm spot.
...
buildLogoGeo(170, geo => {
  mesh = new THREE.Mesh(geo, new THREE.MeshStandardMaterial({ color: 0xffc85e, metalness: 1, roughness: 0.22 }))
  scene.add(mesh)
})
const floor = new THREE.Mesh(new THREE.PlaneGeometry(50, 50),
  new THREE.MeshStandardMaterial({ color: 0x0a0806, metalness: 1, roughness: 0.32 }))
floor.rotation.x = -Math.PI / 2
floor.position.y = -0.62
scene.add(floor)
const spot = new THREE.SpotLight(0xffdf9f, 260, 30, 0.7, 0.5, 2)
spot.position.set(1.5, 4, 2)
scene.add(spot, spot.target)
function tick(dt) {
  if (!mesh) return
  mesh.rotation.y = t * 0.5 + mouseX * 0.3
  mesh.rotation.x = Math.sin(t * 0.3) * 0.05 - mouseY * 0.1
}
