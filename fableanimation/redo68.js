...
// Tusa: a porcelain mark being drowned in ink — a black tide climbs the letters in runny streaks, swallows them, then drains away. White gallery light.
...
let mesh = null
const uni = { uTime: { value: 0 }, uInk: { value: -0.6 } }
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      float diff = max(dot(n, normalize(vec3(0.3, 0.8, 0.6))), 0.0)
                 + 0.4 * max(dot(n, normalize(vec3(-0.5, -0.2, 0.6))), 0.0);
      vec3 porcelain = vec3(0.97, 0.96, 0.93) * (0.55 + 0.45 * diff);
      float runs = 0.14 * vno(vec2(vP.x * 5.0, 3.0)) + 0.05 * vno(vec2(vP.x * 17.0, 9.0));
      float line = uInk + runs;
      float ink = smoothstep(line + 0.02, line - 0.02, -vP.y);
      ink = 1.0 - ink;
      vec3 inked = vec3(0.03, 0.03, 0.045) * (0.8 + 0.4 * diff);
      inked += vec3(1.0) * pow(max(dot(reflect(-v, n), normalize(vec3(0.3, 0.8, 0.6))), 0.0), 50.0) * 0.5;
      vec3 col = mix(porcelain, inked, ink);
...
function tick(dt) {
  ...
  const c = (t % 14) / 14
  let ink
  if (c < 0.4) ink = -0.55 + (c / 0.4) * 1.1        // tide climbs
  else if (c < 0.6) ink = 0.55                       // hold, drowned
  else if (c < 0.9) ink = 0.55 - ((c - 0.6) / 0.3) * 1.1
  else ink = -0.55
  uni.uInk.value += (ink - uni.uInk.value) * Math.min(1, dt * 3)
  if (!mesh) return
  mesh.rotation.y = Math.sin(t * 0.25) * 0.22 + mouseX * 0.3
  mesh.rotation.x = Math.sin(t * 0.18) * 0.06 - mouseY * 0.12
}
