...
**Корневые URL:** домашняя страница «корневого языка» хоста живёт на голом
корне: `redobureau.com/` — английская главная (200, без редиректа на /en),
`redobureau.ru/` — русская. `/en` и `/ru` отдают 301 на `/`. Глубокие URL
сохраняют префикс (`/en/work`, `/ru/work`); испанский всегда с префиксом
(`/es`, `/es/work`). Карта — опция `site.langRoots` в базовом конфиге, её
уважают canonicalUrl/crossLangUrl (hreflang, canonical, sitemap, свитчер).
...
## ⚠️ Железное правило: испанский всегда вместе с английским

Испанская версия — полноценная, НЕ фолбэк. Любое действие с контентом или
кодом на международном сайте выполняется сразу в трёх местах:

| Что делаем | en | es | ru |
|---|---|---|---|
| Новая страница / статья / товар | ✅ мастер | ✅ обязательно сразу | по audience |
| Правка текста на существующей странице | ✅ | ✅ той же правкой | если есть ru-версия |
| Новая UI-строка в шаблоне | `en.php` | `es.php` — в том же коммите | `ru.php` |
| Новый раздел (nav, sitemap, hreflang) | ✅ | автоматически через activeLanguages — проверить глазами | по флагу хоста |

Механика: у каждого контент-файла `*.en.txt` должен существовать парный
`*.es.txt` с переведёнными текстовыми полями (Title, Intro, Desc, Sharetext,
Body-блоки). Поле без перевода тихо фолбэчится на английский — это
считается недоделкой, а не фичей: /es с английскими кусками выглядит
сломанным для испаноязычного клиента и роняет доверие к hreflang у Google.

Чеклист перед публикацией чего угодно на .com:
1. `/en/<страница>` — ок;
2. `/es/<страница>` — переведено всё видимое (не только заголовок);
3. фильтры/теги согласованы со справочниками `site.es.txt` (§ Блок А
   перевода: Industries/Worktypes переведены консистентно с тегами страниц).
...
- **Новая статья в Journal:** панель на .com → Journal → New page →
  заполнить en → переключить язык страницы на Español и заполнить es
  (обязательно, см. железное правило) → при необходимости ru →
  Publish → кнопка Sync для .ru.
